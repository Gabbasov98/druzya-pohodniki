...
function placeSlider() {
    var swiper = new Swiper('.place .swiper-container', {
        slidesPerView: 1,
        spaceBetween: 0,
        navigation: {
            nextEl: '.place .swiper-button-next',
            prevEl: '.place .swiper-button-prev',
        },
        pagination: {
...
filterItems.forEach(filter => {
    ...
    let dropdownToggleBtn = filter.querySelector(".filter-item__toggle")
    dropdownToggleBtn.onclick = function () {
    ...
    if(!filter.classList.contains("_radios")) return
    let dropdownRadios = filter.querySelectorAll(".m-check input")
    let currentValue = filter.querySelector(".filter-item__current")
    ...
    function setCurrentValue() {
        let activeRadio = filter.querySelector(".m-check input:checked")
        currentValue.innerHTML = activeRadio.value
    }
})


let filterToggleBtn = document.querySelectorAll("._toggle-filter")
if(filterToggleBtn){
    filterToggleBtn.forEach(btn => {
        btn.onclick = function () {
            body.classList.toggle("fixed-body")
            body.classList.toggle("_show-filter")
        }
    })
}

let reviewFormToggleBtn = document.querySelectorAll("._toggle-review-form")
if(reviewFormToggleBtn){
    reviewFormToggleBtn.forEach(btn => {
        btn.onclick = function () {
            body.classList.toggle("fixed-body")
            body.classList.toggle("_open-review-form")
        }
    })
}

let orderFormToggleBtn = document.querySelectorAll("._toggle-order-form")
if(orderFormToggleBtn){
    orderFormToggleBtn.forEach(btn => {
        btn.onclick = function () {
            body.classList.toggle("fixed-body")
            body.classList.toggle("_open-order-form")
        }
    })
}
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    let dateFields = document.querySelectorAll("._date-picker")
